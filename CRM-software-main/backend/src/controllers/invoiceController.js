...
export const saveCustomerAndSendWhatsApp = asyncHandler(async (req, res) => {
  ...
  if (send_whatsapp) {
    ...
    const salonAddress = await SettingsModel.getString('salon_address', '');
    const salonPhone = await SettingsModel.getString('salon_phone', '');
    const message = formatInvoiceMessage(invoice, salonName, salonAddress, salonPhone);
